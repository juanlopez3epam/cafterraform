...
variable "private_link_service_network_policies_enabled" {
  description = "(Optional) Enable or Disable network policies for the private link service on the subnet. Default valule is false. Conflicts with private_endpoint_network_policies_enabled."
  type        = bool
  default     = null
}
variable "service_endpoints" {
  ...
  default     = []
  validation {
    condition     = var.service_endpoints == [] || var.service_endpoints == toset([]) || (length(var.service_endpoints) > 0 ? length(setsubtract(var.service_endpoints, ["Microsoft.AzureActiveDirectory", "Microsoft.AzureCosmosDB", "Microsoft.ContainerRegistry", "Microsoft.EventHub", "Microsoft.KeyVault", "Microsoft.ServiceBus", "Microsoft.Sql", "Microsoft.Storage", "Microsoft.Web"])) == 0 : true)
...
variable "settings" {
  description = "Subnet settings object (see module README.md)"
  type        = any
}
